...
impl Mcts {
    ...
    pub fn reset_to(&mut self, exploration_param: f64, board: &Board, color: PieceColor) {
        self.exploration_param = exploration_param;
        self.tree_root = Rc::new(RefCell::new(MctsTreenode::new_root(color, board.clone())));
        self.num_nodes = 0;
    }
    ...
    pub fn grow_with_time_limit(&mut self, time_limit: u128) {
        ...
        while start_time.elapsed().as_millis() < time_limit {
            ...
            if term {
                ...
                let (who_won, _) = node_to_expand_borrowed.rollout_policy();
                let outcome = match node_to_expand_borrowed.get_color() {
                    PieceColor::Attacker => -who_won,
                    PieceColor::Defender => who_won,
                };
            ...
            let (who_won, _) = new_child.rollout_policy();

            let outcome = match new_child.get_color() {
                PieceColor::Attacker => -who_won,
                PieceColor::Defender => who_won,
            };
    ...
    fn tree_policy(&self, expl_param: f64) -> Option<TreenodeRef> {
        ...
        loop {
            ...
            let terminus = RefCell::borrow(&current_node).is_terminal();

            // if the current node is terminal or there are still some moves not expanded -> return
            if i < RefCell::borrow(&current_node).num_movs() || terminus {
...
impl Bot for Mcts {
    ...
    fn get_next_move(
        ...
        self.reset_to(self.exploration_param, board, color.clone());
